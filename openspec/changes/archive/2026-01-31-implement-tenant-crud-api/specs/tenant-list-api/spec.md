...
The system SHALL provide a GET endpoint at `/api/tenants` that returns a list of all tenants.

#### Scenario: Retrieve all tenants
- **WHEN** a client sends a GET request to `/api/tenants`
...
- **THEN** it includes the GET `/api/tenants` endpoint
