...
### Requirement: POST /v1/tenants creates tenant and triggers provisioning workflow
...
- **WHEN** a client sends POST /v1/tenants with valid tenant specification
...
- **WHEN** a client sends POST /v1/tenants with invalid or missing required fields
- **THEN** the API SHALL return HTTP 400 Bad Request without creating a tenant or triggering a workflow

### Requirement: PUT /v1/tenants/{id} updates tenant and triggers update workflow
...
- **WHEN** a client sends PUT /v1/tenants/{id} with updated tenant specification
...
- **WHEN** a client sends PUT /v1/tenants/{id} for a non-existent tenant
...
### Requirement: DELETE /v1/tenants/{id} marks tenant for deletion and triggers deletion workflow
...
- **WHEN** a client sends DELETE /v1/tenants/{id}
...
- **WHEN** a client sends DELETE /v1/tenants/{id} for a non-existent tenant
- **THEN** the API SHALL return HTTP 404 Not Found without triggering a workflow

#### Scenario: Delete on already deleted tenant
- **WHEN** a client sends DELETE /v1/tenants/{id} for a tenant already in "deleted" status
- **THEN** the API SHALL return HTTP 410 Gone without triggering a workflow

### Requirement: GET /v1/tenants returns list of tenants with workflow status
...
- **WHEN** a client sends GET /v1/tenants
...
### Requirement: GET /v1/tenants/{id} returns single tenant with workflow status
...
- **WHEN** a client sends GET /v1/tenants/{id} for a tenant with an active workflow
...
- **WHEN** a client sends GET /v1/tenants/{id} for a tenant in "ready" status
- **THEN** the response SHALL include the tenant record with null workflow_execution_id

#### Scenario: Get non-existent tenant
- **WHEN** a client sends GET /v1/tenants/{id} for a non-existent tenant
