...
The system SHALL provide a PUT or PATCH endpoint at `/api/tenants/{id}` that updates tenant properties.
...
- **WHEN** the update request specifies an existing tenant ID
...
- **WHEN** an update request specifies a tenant ID that does not exist
...
- **THEN** the system validates it against the active provider and updates Tenant.DesiredConfig
